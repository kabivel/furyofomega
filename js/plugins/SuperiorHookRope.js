 ...
 * @desc Set one of them: ok, shift, pageup, or pagedown
 * @default ok
 *
 ...
 *
 * @param Trigger For Hook Rope
 * @desc フックロープ起動条件。以下が有効: ok, shift, pageup, pagedown
 * @default ok
 *
 * @param Switch ID HookRope Works
...
(function() {
  ...
  //
  if (!['ok', 'shift', 'pageup', 'pagedown'].contains(triggerKey)) {
    triggerKey = 'ok';
  };
  ...
  var nonMoveJson = function (distance) {
    return {list:[
      {code:Game_Character.ROUTE_WAIT, parameters:[distance * 4 + 4]},
  ...
  var _isPicketTileThere = function (x, y) {
    return $gameSystem._isPicketTileImage($gameMap.tileId(x, y, 1));
  };
  ...
  var _isHookRopeEventThere = function (x, y) {
    ...
    for (var i = 0; i < events.length; i++) {
      ...
      if (pageOk && $gameSystem.isPicketImage(event.page().image)) {
  ...
  var performHookRopeAnimation = function () {
    ...
    if (type === 0) {
      ...
      case Game_Event._FETCH:
        ...
        var coord = _directionToCoord(dir, distance);
        distance += $gamePlayer._needsBackwardForFetch() ? 1 : 0;
        $gameMap.eventsXy(coord.x, coord.y).forEach(function (event) {
...
//hookRopeSwitchId
  ...
  Game_Player.prototype.triggerButtonAction = function() {
    ...
    if (!someoneInvoked && hookRopeConditionMet()) {
